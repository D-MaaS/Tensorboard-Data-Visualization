...
for dataset in data_dir_list:
    ...
    for img in img_list:
        ...
        input_img_resize=cv2.resize(input_img,(1200,224))
        img_data.append(input_img_resize)
    
                
img_data = np.array(img_data)

#%% code added for dynamic labeling 
def directory(path):
  list_dir = []
  z = 0
  list_dir = os.listdir(path)
  
  for i,dir_name in enumerate(list_dir):
    files = os.listdir(os.path.join(path,dir_name)) # dir is your directory path
    number_files = len(files)
    y[z:z+number_files] = i
    z = z+number_files
  return list_dir

#%% code modified prarthana
  
feature_vectors = np.loadtxt('feature_vectors_900_samples.txt')
...
names = directory(data_path) # ['aa','bb','cc','dd','ee','ff']

#with open(metadata, 'w') as metadata_file:
#    for row in range(210):
#        c = y[row]
#        metadata_file.write('{}\n'.format(c))
metadata_file = open(os.path.join(LOG_DIR, 'metadata_24_classes.tsv'), 'w')
